...
interface CartContextType {
  orders: CartItem[];
  customerName: string;
  deliveryOption: 'pickup' | 'delivery' | 'island_wide' | 'inquiry' | 'overseas';
...
export function CartProvider({ children }: CartProviderProps) {
  ...
  const [deliveryOption, setDeliveryOption] = useState<'pickup' | 'delivery' | 'island_wide' | 'inquiry' | 'overseas'>('pickup');
  ...
  const loadSavedState = () => {
    try {
      ...
      setInquiryMessage(localStorage.getItem('inquiryMessage') || '');
      setParish(localStorage.getItem('parish') || '');
  ...
  const saveState = () => {
    try {
      ...
      localStorage.setItem('inquiryMessage', inquiryMessage);
  ...
  useEffect(() => {
    saveState();
  }, [orders, customerName, deliveryOption, deliveryAddress, pickupTime, inquiryMessage, parish, country]);
  ...
  const updateCustomerInfo = (info: {
    ...
    if (info.inquiryMessage !== undefined) setInquiryMessage(info.inquiryMessage);
    if (info.parish !== undefined) setParish(info.parish);
    if (info.country !== undefined) setCountry(info.country);
  };
