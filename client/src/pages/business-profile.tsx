...
import { useSubmitBusinessReview } from "@/hooks/use-submit-business-review";
...
export default function BusinessProfile({ params }: BusinessProfileProps) {
  ...
  const { submit, isSubmitting } = useSubmitBusinessReview(params.id);
  const { mutate } = useBusinessReviews(params.id);
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-primary text-primary-foreground p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {(business.profileType === 'product_sales' || business.profileType === 'product_inquiry') && (
      ...
      <div className="p-4 space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className="mt-4 space-y-4">
              <BusinessVote businessId={business.id} isComingSoon={status === 'coming_soon'} />
              {status !== 'coming_soon' && (
                <div className="border-t pt-4 space-y-6">
                      ...
                      <BusinessReviewForm
                        ...
                        onSubmit={async (rating, comment) => {
                          try {
                            ...
                            mutate(); // Refresh reviews after submission
                            toast.success('Review submitted successfully');
                          } catch (error) {
                            if (error instanceof Error && error.message === 'Username is required') {
                  ...
                  <div>
                    ...
                    <BusinessReviewList businessId={business.id} />
                  </div>
                </div>
              )}
            </div>
        ...
        {(business.hasDelivery || business.profileType === 'product_inquiry') && status !== 'coming_soon' && (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2">
                <Truck className="h-5 w-5" />
                {business.profileType === 'product_inquiry' ? 'Product Inquiry Information' : 'Delivery Information'}
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              {business.profileType === 'product_inquiry' ? (
                <div className="flex items-start gap-3">
                  <Info className="h-5 w-5 text-blue-600 mt-0.5" />
                  <div>
                    <p className="font-medium">Delivery Available</p>
                    <p className="text-muted-foreground">Delivery option available in inquiry</p>
                  </div>
                </div>
              ) : (
                <>
        ...
        {status !== 'coming_soon' && (
          <div className="pb-6">
            <Button
              ...
              size="lg"
            >
              {business.profileType === 'product_inquiry' ? 'View Products for Inquiry' : 'View Products'}
            </Button>
          </div>
        )}
      </div>
